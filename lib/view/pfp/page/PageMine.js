...
class PageMine {
    ...
    async load(addr, _page) {
        ...
        this.sortor.on("multiple-sell", () => {
            if (this.multipleSelector !== undefined) {
                ...
                this.multipleSelector = new MultiplePFPSelector_1.default(this.addr, "sell").appendTo(this.container);
    ...
    async loadNFTs() {
        ...
        if (address !== undefined) {
            const _totalSupply = (await PageLayout_1.default.current.contract.balanceOf(address)).toNumber();
            const enumerable = await PFPsContract_1.default.enumerables(this.addr);
            if (enumerable === true) {
                const promises = [];
                for (let i = 0; i < _totalSupply; i += 1) {
                    const promise = async (index) => {
                        try {
                            const id = (await PageLayout_1.default.current.contract.tokenOfOwnerByIndex(address, index)).toNumber();
                ...
                const result = await superagent_1.default.get(`https://api.klu.bs/v2/pfp/${this.addr}/owned/${address}`);
    ...
    goPage(page) {
        skyrouter_1.SkyRouter.go(`/pfp/${this.addr}/mine/${page}`);
    }
