...
class Detail {
    constructor(params) {
        ...
        })), (0, skynode_1.el)("select", (0, skynode_1.el)("option", "기본 정렬", { value: "" }), (0, skynode_1.el)("option", "최저가순", { value: "price-asc" }), (0, skynode_1.el)("option", "최고가순", { value: "price-desc" }), {
            change: (event, select) => {
                this.sort = select.domElement.value;
                this.loadNFTs(addr);
            },
    ...
    async loadNFTs(addr) {
        ...
        if (this.idQuery.trim() !== "") {
            ...
            new PFPNFTCard_1.default(addr, parseInt(this.idQuery.trim(), 10)).appendTo(this.nftList);
        }
            ...
            const result = await superagent_1.default.get(`https://api.klu.bs/v2/pfp/${addr}/sales/${this.sort === "price-asc" ? "asc" : "desc"}`);
            ...
            for (let i = start; i < limit; i += 1) {
                new PFPNFTCard_1.default(addr, dataSet[i].nftId).appendTo(this.nftList);
            }
        }
            ...
            if (address !== undefined) {
                ...
                if (enumerable === true) {
                        ...
                        const promise = async (index) => {
                            try {
                                ...
                                if (currentOrder === this.order) {
                                    new PFPNFTCard_1.default(addr, id).appendTo(this.nftList);
                                }
                            }
                            catch (e) {
                                console.error(e);
                    ...
                    const result = await superagent_1.default.get(`https://api.klu.bs/v2/pfp/${addr}/owned/${address}`);
                    const dataSet = result.body;
                    const start = this.page * 50;
                    let limit = (this.page + 1) * 50;
                    if (limit > dataSet.length) {
